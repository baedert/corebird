...
static void
oauth2_proxy_get_property (GObject    *object,
                           guint       property_id,
                           GValue     *value,
                           GParamSpec *pspec)
{
  OAuth2ProxyPrivate *priv = oauth2_proxy_get_instance_private ((OAuth2Proxy *)object);
...
static void
oauth2_proxy_set_property (GObject      *object,
                           guint         property_id,
                           const GValue *value,
                           GParamSpec   *pspec)
{
...
{
  OAuth2ProxyPrivate *priv = oauth2_proxy_get_instance_private (proxy);
  char *url;
  GString *params = 0;
  ...
  encoded_id = soup_uri_encode (priv->client_id, EXTRA_CHARS_ENCODE);

  url = g_strdup_printf ("%s?client_id=%s&redirect_uri=%s&type=user_agent",
                         priv->auth_endpoint, encoded_id,
                         encoded_uri);
...
oauth2_proxy_get_access_token (OAuth2Proxy *proxy)
{
  ...
  g_return_val_if_fail (OAUTH2_IS_PROXY (proxy), "");
